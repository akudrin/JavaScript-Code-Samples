var twitreq = require('./twitreq'); var http = require('http');
...
var query = require('url').parse(req.url,true).query; var search = require('querystring').escape(query.q);
      // forming search path
...
twitreq.getTwitterData(servicePath, atoken, function(results) { res.writeHeader(200, {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"});
         res.end(results);
     });
});
    server.listen(8080);
